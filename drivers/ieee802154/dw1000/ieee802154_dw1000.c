...
#define DW1000_SYSVIEW_MARKER_IRQ_HANDLE  0x7000
#define DW1000_SYSVIEW_MARKER_IRQ_TRIGGER 0x7001
#endif
...
static struct dwt_context dwt_0_context = {
	...
	.pcfg = NULL,  // Will be set by SPI implementation if needed
	.rf_cfg = {
		.channel = 5,
		.dr = DWT_BR_6M8,
		.prf = DWT_PRF_64M,
			    ...
			    size_t buf_len, uint8_t *buf, bool write)
{
	...
	if (write) {
		...
		return dw1000_spi_write(dev, hdr_len, hdr, buf_len, buf);
	} else {
		return dw1000_spi_read(dev, hdr_len, hdr, buf_len, buf);
	}
}

int dwt_register_read(const struct device *dev,
...
inline uint32_t dwt_reg_read_u32(const struct device *dev, uint8_t reg, uint16_t offset)
{
...
inline uint16_t dwt_reg_read_u16(const struct device *dev,
				     uint8_t reg, uint16_t offset)
{
...
inline void dwt_switch_buffers(const struct device *dev) {
...
inline void dwt_fast_enable_rx(const struct device *dev, uint64_t dwt_rx_ts)
{
...
inline void dwt_read_rx_info(const struct device *dev, struct dwt_rx_info_regs *rx_inf_reg) {
	dwt_register_read(dev, DWT_RX_FQUAL_ID, 0, sizeof(struct dwt_rx_info_regs),
			  (uint8_t *)rx_inf_reg);
}

inline uint64_t dwt_rx_timestamp_from_rx_info(const struct dwt_rx_info_regs *rx_inf_reg) {
	uint8_t ts_buf[sizeof(uint64_t)] = {0};
	memcpy(ts_buf, rx_inf_reg->rx_time, DWT_RX_TIME_RX_STAMP_LEN);
	return sys_get_le64(ts_buf);
}

inline uint16_t dwt_fp_index_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16((uint8_t*)rx_inf_reg->rx_time + DWT_RX_TIME_FP_INDEX_OFFSET);
}

inline uint16_t dwt_fp_ampl1_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16((uint8_t*)rx_inf_reg->rx_time + DWT_RX_TIME_FP_AMPL1_OFFSET);
}

inline uint16_t dwt_std_noise_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16((uint8_t*)rx_inf_reg->rx_fqual + DWT_RX_FQUAL_STD_NOISE_OFFSET);
}

inline uint16_t dwt_fp_ampl2_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16((uint8_t*)rx_inf_reg->rx_fqual + DWT_RX_FQUAL_FP_AMPL2_OFFSET);
}

inline uint16_t dwt_fp_ampl3_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16((uint8_t*)rx_inf_reg->rx_fqual + DWT_RX_FQUAL_FP_AMPL3_OFFSET);
}

inline uint16_t dwt_cir_pwr_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return sys_get_le16(&rx_inf_reg->rx_fqual[DWT_RX_FQUAL_FP_CIR_PWR_OFFSET]);
}

inline uint8_t dwt_rcphase_from_info_reg(const struct dwt_rx_info_regs *rx_inf_reg) {
	return rx_inf_reg->rx_ttcko[4] & 0x7F;
}


inline void dwt_double_buffering_align(const struct device *dev) {
	uint32_t sys_stat;
...
static inline void dwt_irq_handle_rx(const struct device *dev, uint32_t sys_stat)
{
	...
	if (IS_ENABLED(CONFIG_NET_PKT_TIMESTAMP)) {
		...
		ctx->rx_ttcko_rc_phase = dwt_rcphase_from_info_reg(&rx_inf_reg);
	}
...
static inline void dwt_irq_handle_error(const struct device *dev, uint32_t sys_stat)
{
	...
	LOG_INF("DW1000 RX ERROR: sys_stat=0x%08x", sys_stat);
	...
	if (sys_stat & DWT_SYS_STATUS_RXPHE) {
		LOG_ERR("RX error DWT_SYS_STATUS_RXPHE (PHY Header Error)");
	}

	if (sys_stat & DWT_SYS_STATUS_RXFCE) {
		LOG_ERR("RX error DWT_SYS_STATUS_RXFCE (Frame Check Sequence Error)");
	}

	if (sys_stat & DWT_SYS_STATUS_RXRFSL) {
		LOG_ERR("RX error DWT_SYS_STATUS_RXRFSL (Reed Solomon Error)");
	}

	if (sys_stat & DWT_SYS_STATUS_RXOVRR) {
		LOG_ERR("RX error DWT_SYS_STATUS_RXOVRR (Overrun Error)");
	}

	if (sys_stat & DWT_SYS_STATUS_RXSFDTO) {
		LOG_ERR("RX error DWT_SYS_STATUS_RXSFDTO (SFD Timeout)");
	}

	if (sys_stat & DWT_SYS_STATUS_AFFREJ) {
		LOG_ERR("RX error DWT_SYS_STATUS_AFFREJ (Automatic Frame Filtering Rejection)");
	}
...
static void dwt_irq_work_handler(struct k_work *item)
{
...
#ifdef CONFIG_SEGGER_SYSTEMVIEW
	SEGGER_SYSVIEW_OnUserStart(DW1000_SYSVIEW_MARKER_IRQ_HANDLE);
#endif
	...
	if (sys_stat & DWT_SYS_STATUS_RXFCG) {
		if (atomic_test_bit(&ctx->state, DWT_STATE_IRQ_POLLING_EMU)) {
			// Signal RX completion for IRQ polling emulation
...
#ifdef CONFIG_SEGGER_SYSTEMVIEW
	SEGGER_SYSVIEW_OnUserStop(DW1000_SYSVIEW_MARKER_IRQ_HANDLE);
#endif
}
			      ...
			      struct gpio_callback *cb, uint32_t pins)
{
	...
	if(pin_state > 0) {
#ifdef CONFIG_SEGGER_SYSTEMVIEW
		SEGGER_SYSVIEW_OnUserStart(DW1000_SYSVIEW_MARKER_IRQ_TRIGGER);
		SEGGER_SYSVIEW_OnUserStop(DW1000_SYSVIEW_MARKER_IRQ_TRIGGER);
#endif
#ifdef CONFIG_IEEE802154_DSKIEL_DW1000_DIRECT_ISR_PROCESSING
		// Direct ISR processing - bypasses work queue for lower latency
		// WARNING: This may cause system instability due to kernel API violations
		dwt_irq_work_handler(&ctx->irq_cb_work);
#else
		// Standard work queue processing - safer but higher latency
...
inline int setup_tx_frame(const struct device *dev, const uint8_t *data, uint8_t len) {
	uint32_t tx_fctrl;
...
inline int dwt_fast_enable_tx(const struct device *dev, uint64_t dwt_tx_ts) {
	struct dwt_context *ctx = dev->data;
...
inline int wait_for_phy(const struct device *dev) {
	struct dwt_context *ctx = dev->data;
...
static void dwt_set_rx_mode(const struct device *dev)
{
	...
	rx_sniff[0] = CONFIG_IEEE802154_DSKIEL_DW1000_SNIFF_ONT &
		      DWT_RX_SNIFF_SNIFF_ONT_MASK;
	/* SNIFF Mode OFF time in microseconds */
	rx_sniff[1] = CONFIG_IEEE802154_DSKIEL_DW1000_SNIFF_OFFT;
...
static int dwt_start(const struct device *dev)
{
	...
	if (CONFIG_IEEE802154_DSKIEL_DW1000_SNIFF_ONT != 0) {
		dwt_set_rx_mode(dev);
	}
...
inline void dwt_enable_accumulator_memory_access(const struct device *dev) {
...
inline void dwt_disable_accumulator_memory_access(const struct device *dev) {
	uint8_t sclks = dwt_reg_read_u8(dev, DWT_PMSC_ID, DWT_PMSC_CTRL0_OFFSET);
...
static int dwt_initialise_dev(const struct device *dev)
{
	...
	/* Setup antenna delay values - priority: OTP > Device Tree > Hardcoded defaults */
	uint16_t tx_ant_delay = DW1000_TX_ANT_DLY;  // Default
	uint16_t rx_ant_delay = DW1000_RX_ANT_DLY;  // Default

#if CONFIG_IEEE802154_DSKIEL_DW1000_OTP_ANTENNA_DELAY
	// OTP takes highest priority when enabled
	tx_ant_delay = otp_antenna_delay & 0xFFFF;
	rx_ant_delay = otp_antenna_delay & 0xFFFF;
	LOG_INF("Using OTP antenna delay: %u", tx_ant_delay);
#else
	// Check if device tree specifies antenna delay values
	if (DT_INST_NODE_HAS_PROP(0, tx_ant_delay)) {
		tx_ant_delay = DT_INST_PROP(0, tx_ant_delay);
		LOG_INF("Using device tree TX antenna delay: %u", tx_ant_delay);
	}
	if (DT_INST_NODE_HAS_PROP(0, rx_ant_delay)) {
		rx_ant_delay = DT_INST_PROP(0, rx_ant_delay);
		LOG_INF("Using device tree RX antenna delay: %u", rx_ant_delay);
	}
#endif

	dwt_set_antenna_delay_rx(dev, rx_ant_delay);
	dwt_set_antenna_delay_tx(dev, tx_ant_delay);
	LOG_INF("Final antenna delay: TX=%u, RX=%u", tx_ant_delay, rx_ant_delay);
...
// TODO: EXPERIMENTATION WITH NTM value for random access outlier detection
...
static int dw1000_init(const struct device *dev)
{
...
#ifdef CONFIG_SEGGER_SYSTEMVIEW
	SEGGER_SYSVIEW_NameMarker(DW1000_SYSVIEW_MARKER_IRQ_HANDLE, "dw1000_irq_handle");
	SEGGER_SYSVIEW_NameMarker(DW1000_SYSVIEW_MARKER_IRQ_TRIGGER, "dw1000_irq_trigger");
#endif
	k_sem_init(&ctx->phy_sem, 0, 1);

	/* Initialize SPI interface */
	if (dw1000_spi_init(dev) != 0) {
...
#ifndef CONFIG_IEEE802154_DSKIEL_DW1000_DIRECT_ISR_PROCESSING
	k_work_queue_start(&dwt_work_queue, dwt_work_queue_stack,
		   K_KERNEL_STACK_SIZEOF(dwt_work_queue_stack),
		   CONFIG_SYSTEM_WORKQUEUE_PRIORITY, NULL);

	k_thread_name_set(&dwt_work_queue.thread, "dw1000_wq");
#endif
	...
	// Register UWB driver abstraction
	extern int uwb_driver_dw1000_init(const struct device *dev);
	int ret = uwb_driver_dw1000_init(dev);
	if (ret != 0) {
		LOG_ERR("Failed to register UWB driver abstraction: %d", ret);
		return ret;
	}
...
DEVICE_DT_INST_DEFINE(0, dw1000_init, NULL,
		    ...
		    POST_KERNEL, CONFIG_IEEE802154_DSKIEL_DW1000_INIT_PRIO,
		    &dwt_radio_api);
#else
NET_DEVICE_DT_INST_DEFINE(0,
		...
		&dwt_0_context,
		&dw1000_0_config,
		CONFIG_IEEE802154_DSKIEL_DW1000_INIT_PRIO,
		&dwt_radio_api,
		IEEE802154_L2,
