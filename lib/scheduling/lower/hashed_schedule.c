...
LOG_MODULE_REGISTER(hashed_schedule);

int mtm_hashed_schedule_create(const struct device *dev, uint64_t rtc_event_time, int phases, int slots_per_phase, uint8_t *payload, size_t payload_size, uint16_t slot_padding_us, struct deca_schedule **schedule)
{
    int ret = 0;
    ...
    /* Compute schedule hash from node list + seed for debug comparison.
     * All nodes with the same node table and event time must get the same hash. */
    uint32_t sched_hash = (uint32_t)(rtc_event_time ^ (rtc_event_time >> 32));
    for (size_t i = 0; i < node_count; i++) {
        sched_hash = sched_hash * 31 + transmit_slot_map[i];
    }
    ...
    for (int k = 0; k < s->slot_count; k++) {
        ...
        if (phase > 0) {
            duration = dwt_calculate_slot_duration(dev,
                slots_per_phase, 0, slot_padding_us);
        } else {
            duration = dwt_calculate_slot_duration(dev,
                0, payload_size, slot_padding_us);
        }
